...
using Base: Indices, OneTo, TupleLL, linearindices, tail, to_shape, make_TupleLL, mapTupleLL,
...
# function Broadcasted(f::Type{F}, args::Args) where {F, Args<:TupleLL}
#     style = _combine_styles(args)
#     Broadcasted{typeof(style), Unknown, Nothing, Nothing, Type{F}, Args}(f, args, nothing, nothing)
# end
# Broadcasted{Style}(f::Type{F}, args::Args) where {Style<:BroadcastStyle, F, Args<:TupleLL} =
#     Broadcasted{Style, Unknown, Nothing, Nothing, Type{F}, Args}(f, args, nothing, nothing)
# Broadcasted{Style,ElType}(f::Type{F}, args::Args) where {Style<:BroadcastStyle, ElType, F, Args<:TupleLL} =
#     Broadcasted{Style, ElType, Nothing, Nothing, Type{F}, Args}(f, args, nothing, nothing)
# Broadcasted{Style,ElType}(f::Type{F}, args::Args, axes::Tuple) where {Style<:BroadcastStyle, ElType, F, Args<:TupleLL} =
#     Broadcasted{Style, ElType, typeof(axes), Nothing, Type{F}, Args}(f, args, axes, nothing)
# Broadcasted{Style,ElType}(f::Type{F}, args::Args, axes::Tuple, indexing) where {Style<:BroadcastStyle, ElType, F, Args<:TupleLL} =
#     Broadcasted{Style, ElType, typeof(axes), typeof(indexing), Type{F}, Args}(f, args, axes, indexing)
...
not_nested(::Nothing)                = true

## Instantiation fills in the "missing" fields in Broadcasted.

instantiate(x) = x
instantiate(tt::TupleLL) = TupleLL(instantiate(tt.head), instantiate(tt.rest))

instantiate(x, axes) = x
instantiate(tt::TupleLL, axes) = TupleLL(instantiate(tt.head, axes), instantiate(tt.rest, axes))
...
cat_nested(fieldextractor, bc::Broadcasted) = cat_nested(fieldextractor, fieldextractor(bc), nothing)
...
cat_nested(fieldextractor, t::Nothing, tail) =
    cat_nested(fieldextractor, tail, nothing)
cat_nested(fieldextractor, t::Nothing, tail::Nothing) = nothing
...
make_makeargs(makeargs, ::Nothing) = makeargs
...
@inline function make_headargs(::Nothing)
...
@inline function make_tailargs(::Nothing)
    return @inline function(tail::Vararg{Any,N}) where N
        tail
    end
end
...
broadcast_all(ffilter::FF, argfilter::AF, ::Nothing) where {FF,AF} = true
...
Base.@propagate_inbounds _getindex(args::TupleLL{<:Any, Nothing}, I, indexing::TupleLL{<:Any, Nothing}) =
...
Base.@propagate_inbounds _getindex(args::TupleLL{<:Any, Nothing}, I, ::Nothing) =
    (_broadcast_getindex(args.head, I),)
Base.@propagate_inbounds _getindex(args::TupleLL{Nothing, Nothing}, I, ::Nothing) = ()

Base.@propagate_inbounds function _broadcast_getindex(bc::BroadcastedF{Style, ElType, N, F, Args}, I::Union{Int,CartesianIndex{N}}) where {Style,ElType,N,F,Args}
    bc.f(_getindex(bc.args, I, bc.indexing)...)
end
Base.@propagate_inbounds function _broadcast_getindex(bc::Broadcasted, I)
    broadcast_skip_axes_instantiation(bc) && return bc.f(_getindex(bc.args, I, bc.indexing)...)
...
eltypes(::TupleLL{Nothing,Nothing}) = Tuple{}
eltypes(t::TupleLL{<:Any,Nothing}) = Tuple{_broadcast_getindex_eltype(t.head)}
...
# flatten_args(bc::Broadcasted) = flatten_args(bc.args)
# flatten_args(::TupleLL{Nothing, Nothing}) = ()
# flatten_args(tt:TupleLL) = (tt.head, flatten_args(tt.rest)...)
# flatten_args(tt:TupleLL{<:TupleLL})     = (flatten_args(tt.head)..., flatten_args(tt.rest)...)
# flatten_args(tt:TupleLL{<:Broadcasted}) = (flatten_args(tt.head)..., flatten_args(tt.rest)...)
...
@inline function copyto!(dest::AbstractArray, bc::Broadcasted{<:Union{Scalar,Unknown},ElType,Nothing,Nothing}) where ElType
    if not_nested(bc)
        if bc.f === identity && bc.args isa TupleLL{<:Any,Nothing} # only a single input argument to broadcast!
...
@inline function copyto!(dest::AbstractArray, bc::Broadcasted{Nothing})
    ...
    if bc.f === identity && bc.args isa TupleLL{<:AbstractArray,Nothing} # only a single input argument to broadcast!
        A = bc.args.head
        if axes(dest) == axes(A)
...
longest_tuple(l::Tuple, t::TupleLL{Nothing})  = l
longest_tuple(l::Tuple, ::Nothing)            = l
longest_tuple(::Nothing, t::TupleLL{<:Broadcasted,Nothing}) = longest_tuple(nothing, t.head.args)
longest_tuple(::Nothing, t::TupleLL{<:Broadcasted}) = longest_tuple(longest_tuple(nothing, t.head.args), t.rest)
longest_tuple(l::Tuple, t::TupleLL{<:Broadcasted,Nothing}) = longest_tuple(l, t.head.args)
